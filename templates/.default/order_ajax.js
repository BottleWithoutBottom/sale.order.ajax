...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        initializePrimaryFields: function () {
            ...
            this.orderID = 'order-form';
            this.orderBlockID = 'order';
        },
        ...
        sendRequest: function (action, actionData) {
            ...
            if (eventArgs.action === 'saveOrderAjax') {
                ...
                var getData = this.getData(eventArgs.action, eventArgs.actionData);
                BX.ajax({
                    method: 'POST',
                    dataType: 'html',
                    url: this.ajaxUrl,
        ...
        changeInputs: function () {
            var inputs = BX(this.orderID).querySelectorAll('[data-change]'), classJS = this;
            inputs.forEach(function (item) {
                BX.bind(item, 'change', BX.proxy(classJS.sendRequest, classJS));
            });
        },

        bindEvents: function () {
            BX.bind(BX(this.orderID).querySelector('[data-save-button]'), 'click', BX.proxy(this.clickOrderSaveAction, this));
        ...
        refreshOrder: function (result) {
            if (typeof(result) == "string") {
                ...
                var htmlContent = doc.querySelector('#' + this.orderID).innerHTML;
                BX(this.orderID).innerHTML = htmlContent;
                this.changeInputs();
                this.prepareLocations(this.allParameters.locations);
            }else{
